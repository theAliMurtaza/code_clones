...
const TYPE_COLORS = {
  'Type-1': { bg: 'rgba(5,150,105,.08)',   border: '#059669', num: '#059669' },
  'Type-2': { bg: 'rgba(37,99,235,.08)',   border: '#2563eb', num: '#2563eb' },
  'Type-3': { bg: 'rgba(217,119,6,.08)',   border: '#d97706', num: '#d97706' },
  'Type-4': { bg: 'rgba(124,58,237,.08)',  border: '#7c3aed', num: '#7c3aed' },
}
...
function ColourLine({ text }) {
  ...
  while ((m = combined.exec(text)) !== null) {
    if (m.index > last) segments.push(<span key={key++} className="text-t1">{text.slice(last, m.index)}</span>)
    const v = m[0]
    if (m[1])  segments.push(<span key={key++} className="text-t3 italic">{v}</span>)
    else if (m[3] !== undefined) segments.push(<span key={key++} className="text-violet font-medium">{v}</span>)
    else if (m[5] !== undefined || m[6] !== undefined) segments.push(<span key={key++} className="text-accent font-medium">{v}</span>)
    else if (m[7] !== undefined) segments.push(<span key={key++} className="text-emerald">{v}</span>)
    else if (m[8] !== undefined) segments.push(<span key={key++} className="text-success">{v}</span>)
    else if (m[9] !== undefined) segments.push(<span key={key++} className="text-amber">{v}</span>)
    last = m.index + v.length
  }
  if (last < text.length) segments.push(<span key={key++} className="text-t1">{text.slice(last)}</span>)
  return <>{segments.length ? segments : <span className="text-t1">{text}</span>}</>
}
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-s2 border-b border-b1 flex-shrink-0">
        ...
        {lang && <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 rounded bg-s3 text-t3 uppercase">{lang}</span>}
        <span className="text-[9px] font-mono text-t3 flex-shrink-0">{lines.length} lines</span>
      </div>

      {/* Code body */}
      <div className="overflow-auto flex-1 min-h-0 bg-surface">
            ...
            {lines.map(({ n, text }) => {
              ...
              return (
                <tr key={n} style={isClone ? { background: colors.bg } : undefined}
                  className={isClone ? '' : 'hover:bg-s2/60'}>
                  {/* Line number gutter */}
                  <td className="select-none text-right pr-3 pl-3 py-0.5 min-w-[44px] border-r leading-6 sticky left-0"
                    style={{
                      borderColor: isClone ? colors.border : '#e2e8f0',
                      borderLeftWidth: isClone ? 2 : 0,
                      background: isClone ? colors.bg : '#f8fafc',
                      color: isClone ? colors.num : '#94a3b8',
